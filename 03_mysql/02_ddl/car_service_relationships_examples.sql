-- DDL relationships lab — parent/child, many-to-many, self-reference
-- Usage: mysql -u... -p... < car_service_relationships_examples.sql
-- Runs in sandbox DB: ddl_practice
...
-- Reset only relationship demo objects.
...
-- 2) Self-reference (employee -> manager)
...
-- 3) One-to-many (customer -> work orders)
...
-- 4) Reference table (parts catalog)
...
-- 5) Many-to-many bridge (work order <-> part)
...
-- Quick verification query (join across 1:N and N:M)
