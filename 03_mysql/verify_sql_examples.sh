...
#   - For algebra + DQL: load 01_database_mysql/car_service_db.sql.gz into `car_service_db` first, e.g.:
...
if ! "${MYSQL[@]}" -Nse "SHOW DATABASES LIKE 'car_service_db'" | grep -q .; then
  ...
  echo "Load: gunzip -c 01_database_mysql/car_service_db.sql.gz | mysql -u $MYSQL_USER ... car_service_db"
...
run_file "Isolation levels (car_service_db)" "$ROOT/09_transactions/car_service_isolation_levels_examples.sql"
