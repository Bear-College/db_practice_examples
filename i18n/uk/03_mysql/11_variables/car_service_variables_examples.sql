-- Змінні користувача та системні — car_service_db (MySQL 8+ / 5.7+)
-- Запускайте весь файл в ОДНІЙ сесії mysql, щоб @-змінні зберігалися між інструкціями.
--   mysql ... car_service_db < 11_variables/car_service_variables_examples.sql

USE car_service_db;

-- =============================================================================
-- 1) Змінні користувача: SET … потім використати в WHERE
-- =============================================================================
SET @wo_lo := 1;
SET @wo_hi := 800;
...
-- =============================================================================
-- 2) Присвоєння з запиту (агрегат у @avg_cost)
-- =============================================================================
...
-- =============================================================================
-- 3) LIMIT / OFFSET через змінні користувача (параметри пагінації)
-- =============================================================================
SET @page_size := 12;
SET @page := 2;
...
-- =============================================================================
-- 4) := у SELECT — класичний лічильник рядків (до появи ROW_NUMBER у віконних)
-- =============================================================================
SELECT id,
       total_cost,
       @rn := @rn + 1 AS row_seq
...
-- =============================================================================
-- 5) Кілька присвоєнь в одному SET + повторне використання
-- =============================================================================
...
-- =============================================================================
-- 6) Підготовлена інструкція: плейсхолдери заповнюються зі змінних користувача
-- =============================================================================
...
-- =============================================================================
-- 7) Системні / статусні змінні (@@) — у типових лабораторіях лише на читання
-- =============================================================================
SELECT "mysql_version" AS k, @@version AS v
UNION ALL
SELECT "tx_isolation" AS k, @@session.transaction_isolation AS v;

-- Опціонально: тимчасово обмежити кількість повернутих рядків (лише сесія); розкоментуйте, щоб спробувати, потім відновіть.
-- SET SESSION sql_select_limit = 50;
-- SELECT id FROM work_orders WHERE id BETWEEN 1 AND 1000 ORDER BY id;
-- SET SESSION sql_select_limit = DEFAULT;

-- =============================================================================
-- 8) Очистити змінну користувача (або задати NULL)
-- =============================================================================
SET @avg_cost := NULL;
