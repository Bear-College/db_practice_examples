-- Збережені процедури — car_service_db (MySQL 5.7+ / 8.x)
...
-- P1 — IN-параметри: повертає набір результатів (з обмеженням)
...
-- P2 — OUT-параметр: агрегат у користувацьку змінну
...
-- P4 — INOUT: змінити сесійну користувацьку змінну, передану в CALL
...
-- -----------------------------------------------------------------------------
-- Виклики (та сама сесія, щоб @-змінні працювали)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Опціонально: видалити процедури по завершенні
-- -----------------------------------------------------------------------------
-- DROP PROCEDURE IF EXISTS sp_work_orders_by_status;
-- DROP PROCEDURE IF EXISTS sp_sum_cost_slice;
-- DROP PROCEDURE IF EXISTS sp_customer_count;
